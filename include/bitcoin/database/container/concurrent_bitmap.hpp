...
#ifndef LIBBITCOIN_MVCC_CONCURRENT_BITMAP_HPP
#define LIBBITCOIN_MVCC_CONCURRENT_BITMAP_HPP

#include <cstring>
#include <memory>

#include <bitcoin/database/container/raw_bitmap.hpp>
...
/**
 * A raw_concurrent_bitmap is a bitmap that does not have the compile-time
 ...
 * raw_concurrent_bitmap. Reinterpret an existing block of memory that
 * you know will be a valid bitmap.
 *
 * Use @see BitmapSize to get the correct size for a bitmap of
 ...
 * deallocate methods.
 *
 * We require raw_concurrent_bitmap to be always aligned to 64-bits on byte 0.
 */
class raw_concurrent_bitmap {
public:

    // Disallow creating and deleting the bitmap. It can only be
    // interpreted as a bitmpa from already allocated memory.
    raw_concurrent_bitmap() = delete;
    ~raw_concurrent_bitmap() = delete;

    /**
     * Allocates a new raw_concurrent_bitmap of size num_bits. Up to
     * the caller to call Deallocate on its return value
     * @param num_bits number of bits (elements to represent) in the bitmap
     * @return ptr to new raw_concurrent_bitmap
     */
    static raw_concurrent_bitmap *allocate(const uint32_t num_bits)
    {
        uint32_t num_bytes = raw_bitmap::size_in_bytes(num_bits);
        auto *result = allocation_util::allocate_aligned(num_bytes);
        BITCOIN_ASSERT_MSG(
            reinterpret_cast<uintptr_t>(result) % sizeof(uint64_t) == 0,
            "Allocate should be 64-bit aligned.");
        std::memset(result, 0, num_bytes);
        return reinterpret_cast<raw_concurrent_bitmap *>(result);
    }

    /**
     * Deallocates a raw_concurrent_bitmap. Only call on pointers given
     * out by Allocate
     * @param map the map to deallocate
     */
    static void deallocate(raw_concurrent_bitmap *const map)
    {
    ...
    bool flip(const uint32_t pos, const bool expected_val)
    {
             ...
             old_val = bits_[element])
        {
            ...
            if (bits_[element].compare_exchange_strong(old_val, new_val))
                return true;
        }
        return false;
    }
     ...
     */
    bool first_unset_pos(const uint32_t bitmap_num_bits, const uint32_t start_pos, uint32_t *const out_pos) const
    {
        ...
        BITCOIN_ASSERT_MSG(
            reinterpret_cast<uintptr_t>(bits_) % sizeof(uint64_t) == 0,
            "bits_ should be 64-bit aligned.");

        // maximum number of bytes in the bitmap
        const uint32_t num_bytes = raw_bitmap::size_in_bytes(bitmap_num_bits);
        // current byte position
        uint32_t byte_pos = start_pos / BYTE_SIZE;
        // number of bits remaining
        uint32_t bits_left = bitmap_num_bits - start_pos;
        // whether we found an unset bit previously
        ...
        while (byte_pos < num_bytes && bits_left > 0) {
            ...
            if (!found_unset_bit &&
                is_aligned_and_fits<uint64_t>(bits_left, byte_pos)) {
                found_unset_bit = find_unset_bit<int64_t>(&byte_pos, &bits_left);
            } else if (!found_unset_bit &&
                is_aligned_and_fits<uint32_t>(bits_left, byte_pos)) {
                found_unset_bit = find_unset_bit<int32_t>(&byte_pos, &bits_left);
            } else if (!found_unset_bit &&
                     is_aligned_and_fits<uint16_t>(bits_left, byte_pos)) {
                found_unset_bit = find_unset_bit<int16_t>(&byte_pos, &bits_left);
            } else {
                ...
                if (static_cast<std::atomic<int8_t>>(bits) != -1) {
                    ...
                    // if the bit is valid, we return the bit. however, the bit could
                    // be invalid:
                    // 1. it could be part of our end padding, 2. it could be before
                    // start_pos in which case we must continue to search the next
                    // byte, if it exists. we don't bother ensuring result freshness
                    // since our returned position is immediately stale.
    ...
    void unsafe_clear(const uint32_t num_bits) {
        auto size = raw_bitmap::size_in_bytes(num_bits);
...
private:
     ...
     */
    template <class T>
    bool find_unset_bit(uint32_t *const byte_pos, uint32_t *const bits_left) const
    {
        BITCOIN_ASSERT_MSG(*bits_left >= sizeof(T) * BYTE_SIZE,
     ...
     */
    template <class T>
    bool is_aligned_and_fits(const uint32_t bits_left, const uint32_t byte_pos) const
    {
        return bits_left >= sizeof(T) * BYTE_SIZE && byte_pos % sizeof(T) == 0;
    }
};

// WARNING: DO NOT CHANGE THE CLASS LAYOUT OF raw_concurrent_bitmap.
...
static_assert(sizeof(raw_concurrent_bitmap) == 0, "Unexpected raw_concurrent_bitmap layout!");
